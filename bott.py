...
elif query.data.startswith("confirm_crypto_"):
...
elif query.data in ["admin_all_orders", "admin_pending", "admin_paid", "admin_completed", "admin_stats"]:
    if user_id != ADMIN_ID:
        await query.answer("⛔ Нет доступа", show_alert=True)
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif state == 'waiting_funpay_link':
